...
* **Completion:** Once all rounds and days are processed, it calls `Merge Results` to combine the data and triggers `Write Output File` to produce a consolidated log (e.g., `2026-03-01_08-35-51.log` containing `File Details`).

#### 2. The `TestRunner` (Daily Simulator)
The `TestRunner` is responsible for simulating the market environment for a single day:
* **Read Data:** It triggers `Read Data` which calls the `BackDataReader` to read the market data from a file. The reader parses files like `prices_round_1_day_0.csv` and `trades_round_1_day_0.csv` and returns a `BackData` object . This step yields `Result - 0`.
* **Timestamp Loop:** For each timestamp in the loaded data (`for each timestamp`), the runner executes a sequence of events:
    ...
    3. **Algorithm Execution:** The user's `Algorithm` processes the state and returns proposed orders (`Return`). Any standard output (`stdout`) generated by the algorithm is captured as `lambda_log`. This execution step yields `Result - 1`.
    4. **Create Activity Logs:** The `ActivityLogCreator` steps in to record the actions, orders, and market state of the current timestamp. This yields `Result - 2`.
    5. **Match Orders:** The proposed orders are passed to the `OrderMatchMaker`, which simulates the exchange mechanics to fill orders against the historical order book. This yields `Result - 3`.
* **Results Aggregation:** After the timestamp loop concludes, the overall day's simulation yields `Result - 4`, which is returned back to the `BackTester`.
